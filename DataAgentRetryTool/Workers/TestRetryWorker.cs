...
public class TestRetryWorker
{
    ...
    public TestRetryWorker(DataTransmissionService service, DatabaseService databaseService)
    {
        _service = service;
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var countResult = await _service.QueryTestRecordsAsync(1, 10);
                ...
                while (currentPage >= 1 && !cancellationToken.IsCancellationRequested)
                {
                    ...
                    var result = await _service.QueryTestRecordsAsync(currentPage, 10);
    ...
    private string? MatchFilterRules(string errorMessage)
    {
        var filterRules = new[]
        {
            ...
            "SQL序号【89】、数据类型名称【KeyInfo】执行失败：不能在具有唯一索引\"wldwzl_wldwname_uq\"的对象\"dbo.wldwzl\"中插入重复键的行",
        ...
        // 测试环境额外规则：用户登录失败
        if (errorMessage.Contains("登录失败", StringComparison.OrdinalIgnoreCase))
        {
            return "用户登录失败";
        }

        // 测试环境额外规则：字符串截断
        if (errorMessage.Contains("字符串或二进制数据将在", StringComparison.OrdinalIgnoreCase)
            && errorMessage.Contains("中被截断", StringComparison.OrdinalIgnoreCase))
        {
            return "字符串或二进制数据截断";
        }
        ...
        // 测试环境额外规则：SqlDateTime溢出
        if (errorMessage.Contains("SqlDateTime overflow", StringComparison.OrdinalIgnoreCase)
            && errorMessage.Contains("Must be between", StringComparison.OrdinalIgnoreCase))
        {
            return "SqlDateTime溢出";
        }
