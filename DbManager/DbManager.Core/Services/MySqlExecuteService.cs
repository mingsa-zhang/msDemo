...
public class MySqlExecuteService : IDbExecuteService
{
    private readonly IDbConnectionFactory _connectionFactory;

    public MySqlExecuteService(IDbConnectionFactory connectionFactory)
    {
        ...
        string connectionString, string sql, int pageIndex, int pageSize, int? timeout = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            using var dataCmd = new MySqlCommand(pagedSql, conn);
            if (timeout.HasValue)
        ...
        {
            System.Diagnostics.Debug.WriteLine($"MySQL分页查询失败: {ex.Message}");
