...
public class PostgreSqlExecuteService : IDbExecuteService
{
    private readonly IDbConnectionFactory _connectionFactory;

    public PostgreSqlExecuteService(IDbConnectionFactory connectionFactory)
    {
        ...
        string connectionString, string sql, int pageIndex, int pageSize, int? timeout = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            using var dataCmd = new NpgsqlCommand(pagedSql, conn);
            if (timeout.HasValue)
        ...
        {
            System.Diagnostics.Debug.WriteLine($"PostgreSQL分页查询失败: {ex.Message}");
